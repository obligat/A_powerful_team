/**
 * Created by myc on 10/1/16.
 */
export default {
    Activity: [
        {
            activityName: '阳光杯棋奕赛',
            activityTime: '今天',
            activityLocation: '西安邮电大学体育馆内',
            instruction: '切磋棋艺',
            comment: {
                clubName: '奕林社',
                message: '欢迎参加我社举办的棋艺比赛'
            }
        },
        {
            activityName: '轮滑——滑出精彩',
            activityTime: '明天',
            activityLocation: '西北大学操场',
            instruction: '每一次的跌倒不是失败，而是走向青春舞台的必经之路！来吧，只要你想轮滑，只要你喜欢轮滑，还在等什么？亮出追风的姿态，出发吧！',
            comment: {
                clubName: '轮滑社',
                message: '一起滑翔'
            }
        },
        {
            activityName: `let's cos,let's show`,
            activityTime: 'today',
            activityLocation: '陕西师范大学南校区操场',
            instruction: '动漫真人秀（cosplay）表演',
            comment: {
                clubName: 'vivid动漫社',
                message: '一起cos,一起show!'
            }
        },
        {
            activityName: '醉美端阳 戏话白蛇',
            activityTime: 'today',
            activityLocation: '江苏大学大学生活动中心',
            instruction: '展示中华戏曲魅力 ，弘扬民俗文化',
            comment: {
                clubName: '曲艺社',
                message: '欢迎'
            }
        },
        {
            activityName: '8分钟约会',
            activityTime: 'today',
            activityLocation: '山东大学中心校区星咖啡餐厅',
            instruction: '异域风情面对面，缘定你我八分钟',
            comment: {
                clubName: '中外文化交流协会',
                message: '在这个单身骄傲的日子，等你结识更多的朋友'
            }
        },
        {
            activityName: '“萌化大学”创意绘画大赛',
            activityTime: 'today',
            activityLocation: '合肥工业大学翡翠湖校区5教208',
            instruction: '培养同学们的创新精神和动手能力，增强对学校的热爱和归属感',
            comment: {
                clubName: '社团联合会',
                message: '热爱校园，亮出你的作品'
            }
        },
        {
            activityName: '机智应对，密室脱险',
            activityTime: 'today',
            activityLocation: '西安政法大学A栋教学楼',
            instruction: '它不仅考验同学们的智力，更考验同学们在场的应变能力',
            comment: {
                clubName: '心协',
                message: '欢迎来挑战'
            }
        },
        {
            activityName: '“一触即发”联动装置大赛',
            activityTime: 'today',
            activityLocation: '西安理工大学操场',
            instruction: '一触即发的多米诺，梦幻十足的3D投影',
            comment: {
                clubName: '无线电协会',
                message: '期待你的奇思妙想'
            }
        }
    ],
    Community: [
        {
            username: 'myc123',
            password: '123456',
            schoolName: '西安邮电大学',
            communityName: '奕林社',
            logo: 'images/chess.jpg',
            description: `我们棋社是一个积极向上，朝气蓬勃的集体，
            以棋会友,以棋会师是我们的口号，我们的棋技会在这里提高，
            我们的友谊会在这里升华，只要我们努力学棋，就会创造出棋社的辉煌。`,
            email: '12345678911',
            tel: '15829363000'
        },
        {
            username: 'myc123',
            password: '123456',
            schoolName: '西安政法大学',
            communityName: '大学生心理健康协会',
            logo: 'images/mental.png',
            description: `西安政法大学大学生心理协会成立于2006年11月，
            宗旨是“自解困惑，帮助他人”，是由热衷于心理健康教育的学生组成的
            学生社团，在大学生心理健康教育中心的指导下和学生社团联合会的管理、
            监督下开展活动。`,
            email: '12345678911',
            tel: '15829363000'
        },
        {
            username: 'myc123',
            password: '123456',
            schoolName: '江苏大学',
            communityName: '曲艺社',
            logo: 'images/drama.jpg',
            description: ` 曲艺社由来自各个学院爱好相声曲艺的同学组成，
            以弘扬传统艺术为宗旨，以相声为主，其它曲艺形式和戏曲为辅进行发展。
            多次活跃在校园各类文化活动中，为同学们带来欢乐的笑声。`,
            email: '12345678911',
            tel: '15829363000'
        },
        {
            username: 'myc123',
            password: '123456',
            schoolName: '西北大学',
            communityName: '轮滑社',
            logo: 'images/skating.png',
            description: `轮滑社是集健身、竞技、娱乐、趣味、艺术、休闲于一体的学生社团,
                          本社团为轮滑爱好者提供交流平台，培养大学生身体素质、丰富大学生
                          的课余生活，推广轮滑运动.
                          我们的口号是：轮子上的青春，让你我一起滑动。`,
            email: '12345678911',
            tel: '15829363000'
        },
        {
            username: 'myc123',
            password: '123456',
            schoolName: '陕西师范大学',
            communityName: 'vivid动漫社',
            logo: 'images/comic.png',
            description: `vivid动漫社现由秘书部、COS部、手绘部、摄影部和后勤部组成。
                        COS部门主要教化妆服饰动漫，摄影部负责主要的PS工作与讲解，
                        另外，动漫社常在YY上进行配音课堂活动。`,
            email: '12345678911',
            tel: '15829363000'
        },
        {
            username: 'myc123',
            password: '123456',
            schoolName: '合肥工业大学',
            communityName: '社团联合会',
            logo: 'images/association.jpg',
            description: `校社联对校内学生社团进行指导，监督和管理，
            通过组织学生社团广泛开展丰富多彩健康向上的活动，以达到繁荣校园文化的目的。
            包括：
               办公室         负责校社联内部事务的综合性部门，是连接各部门的纽带。
               对外合作交流部  加强社团与外校的合作、交流，演出。
               宣传策划部      做好社团活动的宣传设计、制作及活动布景，并且致力于
                             传播社团信息，传承社团文化，力求营造良好的社团文化氛围。`,
            email: '12345678911',
            tel: '15829363000'
        }
    ]
};